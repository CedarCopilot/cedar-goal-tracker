...
import {
  ChatInputSchema,
  ChatOutputSchema,
  dailyGoalChatWorkflow,
} from './workflows/dailyGoalChatWorkflow';
  ...
  registerApiRoute('/chat', {
    ...
    handler: async (c) => {
      try {
        ...
        const run = await dailyGoalChatWorkflow.createRunAsync();
        ...
        if (result.status === 'success') {
          ...
          return c.json<z.infer<typeof ChatOutputSchema>>(result.result);
        }
      } catch (error) {
        console.error(error);
